...
const DIRTY_MOCK_STATES = new Set();
...
const MOCKS_BY_STATE = new WeakMap();
function createMockInstance(options = {}) {
	...
	const updateMockPrototype = () => {
		if (!options.prototypeMembers?.length) {
			reparentMockPrototype(mock, config.onceMockImplementations[0] || config.mockImplementation || originalImplementation);
		}
	};
...
function spyOn(object, key, accessor) {
	...
	try {
		...
		if (error instanceof TypeError && Symbol.toStringTag && object[Symbol.toStringTag] === "Module" && (error.message.includes("Cannot redefine property") || error.message.includes("Cannot replace module namespace") || error.message.includes("can't redefine non-configurable property"))) {
			throw new TypeError(`Cannot spy on export "${String(key)}". Module namespace is not configurable in ESM. See: https://vitest.dev/guide/mocking/modules#mocking-a-module`, { cause: error });
		}
		throw error;
	}
	return mock;
