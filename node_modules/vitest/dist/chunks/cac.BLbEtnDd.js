import { n as normalize, t as toArray } from './pathe.M-eThtNZ.DwEga6ro.js';
...
const cliOptionsConfig = {
	...
	coverage: {
		...
		subcommands: {
			...
			autoAttachSubprocess: { description: "Track coverage of the `node:child_process` and `node:worker_threads` spawned during test run. Supported only by `v8` provider. (default: false)" }
		}
	},
	mode: {
	...
	sequence: {
		...
		subcommands: {
			...
			hooks: {
				description: "Changes the order in which hooks are executed. Accepted values are: \"stack\", \"list\" and \"parallel\". Visit [`sequence.hooks`](https://vitest.dev/config/sequence#sequence-hooks) for more information (default: `\"stack\"`)",
				argument: "<order>"
			},
			setupFiles: {
				description: "Changes the order in which setup files are executed. Accepted values are: \"list\" and \"parallel\". If set to \"list\", will run setup files in the order they are defined. If set to \"parallel\", will run setup files in parallel (default: `\"list\"`)",
				argument: "<order>"
			},
			groupOrder: null
		}
	...
	diff: {
		...
		subcommands: {
			...
			expand: { description: "Expand all common lines (default: `false`)" },
			...
			printBasicPrototype: { description: "Print basic prototype Object and Array (default: `false`)" },
			maxDepth: {
	...
	project: {
		shorthand: "p",
		description: "The name of the project to run if you are using Vitest workspace feature. This can be repeated for multiple projects: `--project=1 --project=2`. You can also filter projects using wildcards like `--project=packages*`, and exclude projects with `--project=!pattern`. A project runs if it matches no negated pattern and, when regular patterns are also given, matches at least one of them.",
		argument: "<name>",
		array: true
	},
	slowTestThreshold: {
	...
	mergeReports: {
		...
		argument: "[path]",
		transform(value) {
			if (!value || typeof value === "boolean") return ".vitest/blob";
			return value;
		}
	},
	listTags: {
	...
	sharedViteServer: { description: "Let inline projects that don't modify the Vite config reuse the Vite server of the config that declares them. (default: `true`)" },
	experimental: {
		...
		subcommands: {
			...
			diagnostics: {
				...
				argument: "",
				subcommands: {
					isolate: { description: "Print a hint estimating how much time `isolate: false` would save when `isolate: true` spends a significant amount of time spawning a worker per test file. (default: `true`)" },
					environment: { description: "Print a hint when re-creating a DOM environment for every test file dominates the run and a `vm` pool would set it up once per worker. (default: `true`)" },
					import: { description: "Print a hint when test files repeatedly evaluate the same module graph (typical for barrel-file imports) and `isolate: false` would evaluate it once per worker. (default: `true`)" },
					transform: { description: "Print a hint when transforming modules dominates the run and `fsModuleCache` would persist the results across runs. (default: `true`)" }
				}
			}
		}
	},
...
function __${r}_perform_completion
    ...
    __${r}_debug "Calling ${n} complete -- $args[2..-1] $lastArg"
    set -l results (${n} complete -- $args[2..-1] "$lastArg" 2> /dev/null)
...
const o=process.execPath,s=process.argv.slice(1),c=/<[^>]+>|\[[^\]]+\]/;async function l(l,u$1){for(let e of [l.globalCommand,...l.commands]){if(e.name===`complete`)continue;let t=(e.rawName.match(/\[.*?\]|<.*?>/g)||[]).map(e=>e.startsWith(`[`)),n=e.name===`@@global@@`,r=n?u$1:u$1?.subCommands?.[e.name],a=n?``:e.name,o=n?u:u.command(a,e.description||``);if(o){let n=(e.rawName.match(/<([^>]+)>|\[\.\.\.([^\]]+)\]/g)||[]).map(e=>e.startsWith(`<`)&&e.endsWith(`>`)?e.slice(1,-1):e.startsWith(`[...`)&&e.endsWith(`]`)?e.slice(4,-1):e);t.forEach((e,t)=>{let i=n[t]||`arg${t}`,a=r?.args?.[i];a?o.argument(i,a,e):o.argument(i,void 0,e);});}for(let t of [...l.globalCommand.options,...e.options]){let e=t.rawName.match(/^-([a-zA-Z]), --/)?.[1],a=t.name,s=n?u:o;if(s){let n=r?.options?.[a],i=t.required||c.test(t.rawName);n?e?s.option(a,t.description||``,n,e):s.option(a,t.description||``,n):i?e?s.option(a,t.description||``,async()=>[],e):s.option(a,t.description||``,async()=>[]):e?s.option(a,t.description||``,e):s.option(a,t.description||``);}}}return l.command(`complete [shell]`).action(async(c,u$1)=>{let d=l.name;switch(c){case `zsh`:{let t$1=t(l.name,d);console.log(t$1);break}case `bash`:{let e=n(l.name,d);console.log(e);break}case `fish`:{let e=r(l.name,d);console.log(e);break}case `powershell`:{let e=i(l.name,d);console.log(e);break}default:{e(l.name);let e$1=u$1[`--`]||[];return l.showHelpOnExit=false,l.unsetMatchedCommand(),l.parse([o,s[0],...e$1],{run:false}),u.parse(e$1)}}}),u}
...
function createCLI(options = {}) {
	...
	cli.command("doctor [...filters]", void 0, options).action(doctorCommand);
...
async function start(cliFilters, options) {
	try {
		const { startVitest } = await import('./cli-api.DL4Bd7Vh.js').then(function (n) { return n.a; });
		const ctx = await startVitest(cliFilters.map(normalize), normalizeCliOptions(cliFilters, options));
		if (!ctx.shouldKeepServer()) await ctx.exit();
	} catch (e) {
		const { errorBanner } = await import('./utils.CJ0JImL8.js').then(function (n) { return n.u; });
...
async function doctorCommand(cliFilters, options) {
	try {
		const { doctor } = await import('./doctor.BCXa6-1R.js');
...
async function init(project) {
	...
	const { create } = await import('./creator.2VTnpYzN.js');
	await create();
}
async function collect(cliFilters, options) {
	try {
		const { prepareVitest, processCollected, outputFileList } = await import('./cli-api.DL4Bd7Vh.js').then(function (n) { return n.a; });
